...
#     def __init__(self, port_name, chunk_size=1024):
#         super().__init__()
...
#         while True:  # Loop to continuously listen for incoming data
#             if self.serial_port.waitForReadyRead(100):
#                 header = self.serial_port.readLine()  # Read until newline to get the header
#                 if header:
#                     try:
#                         file_type, file_size, chunk_size = header.data().decode('utf-8').strip().split('|')
#                         file_size = int(file_size)
#                         chunk_size = int(chunk_size)
#                         print(f"Receiving {file_type} of size {file_size} bytes with chunk size {chunk_size}")

#                         self.receive_file(file_type, file_size, chunk_size)
#                     except Exception as e:
#                         self.error_occurred.emit(f"Error processing received data: {str(e)}")

#     def receive_file(self, file_type, file_size, chunk_size):
#         """Receive the file data."""
#         received_bytes = 0
#         filename = f"received_file.{file_type}"
...
#                 if not chunk:
#                     self.error_occurred.emit("Connection lost before complete file received.")
#                     break
...
#                 if elapsed_time > 0:
#                     speed_kbps = (received_bytes / 1024) / elapsed_time  # Speed in KB/s
#                     self.speed_signal.emit(speed_kbps)

#                 # Emit progress
#                 self.progress.emit(int((received_bytes / file_size) * 100))

#         print("File received successfully!")
#         self.file_size.emit(str(self.format_size(file_size)))  # Emit the total file size
...
class DataReceiverWorker(QObject):
    ...
    def start_receiving(self):
        ...
        while True:  # Loop to continuously listen for incoming data
            if self.serial_port.waitForReadyRead(100):
                ...
                if data:
                    print(data)  # Debug: Print the raw byte data
                    try:
                        # Try to decode using UTF-8 and replace problematic bytes
    ...
    def close_port(self):
        ...
        if self.serial_port.isOpen():
            self.serial_port.close()
            print(f"Port {self.serial_port.portName()} closed.")
...
#         self.serial_port.setReadBufferSize(1024)  # Set buffer size to 4KB, adjust as needed
...
#         while self.is_running:  # Loop to continuously listen for incoming data
#             if self.serial_port.waitForReadyRead(100):
#                 data = self.serial_port.readAll()
#                 if data:
#                     print(data)  # Debug: Print the raw byte data
...
class DataReceiverWorker(QObject):
    error_occurred = pyqtSignal(str)
    data_received = pyqtSignal(str)  # Signal to emit the received data as a string
    finished = pyqtSignal()  # Signal to indicate the receiving process has finished
    progress = Signal(int)
    ...
    def __init__(self, port_name):
        ...
        # Variables to keep track of the data and header
        self.expected_file_size = 0 
        self.received_data = b""
        self.header_buffer = ""  # Buffer to hold incoming header data
        self.in_header = True
                ...
                if data:  # If data is received, process it
                    ...
                    if self.in_header:
                        ...
                        if "END_HEADER" in self.header_buffer:
                            ...
                            self.in_header = False  # Move to data receiving stage
                            ...
                            self.header_buffer = ""
                            self.progress.emit(0)  # Emit initial progress as 0%
                        continue  
